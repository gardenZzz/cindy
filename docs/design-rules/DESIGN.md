...
- **Mobile-download QR brand edge** — the fourth sanctioned motion class and the only sanctioned *persistent* one (approved 2026-07-25), **only** for the QR card in `MobileDownloadDialog` (brand-surface registration: §15.7):
  - **Definition**: the official app icon sits scaled behind the QR card and is clipped to a 2px hairline edge; it turns a full circle every **20s** (`--mobile-download-edge-cycle`, linear, infinite), so the artwork's own red/blue drifts along the card edge. The dialog is a promotion surface for the mobile app — the slow drift is what marks the QR as "ours" without adding any component-authored color.
  - **Parameter red lines**: the animation touches **only `transform`** on an HTML wrapper (compositor-only, engineering conventions §7 — no `filter` / `backdrop-filter` / `mask-image`, nothing on SVG, no per-frame background work); one cycle length, linear, no easing games; registered in the `prefers-reduced-motion` whitelist in `globals.css`, where it degrades to a static angle. The card carries **no shadow and no pointer tilt** — both were evaluated and removed the same day; do not re-add them under this exception.
  - **Scope boundary**: this card only. Persistent motion elsewhere stays forbidden by the red lines above; a new persistent animation requires a new entry here. Implementation: `.mobile-download-qr-edge` in `globals.css`; contract test: `mobileDownloadDialog.test.tsx` → `keeps the brand edge inside its registered exception`.
...
- **Where**: `components/sidebar/MobileDownloadDialog.tsx` only — the dialog header icon (64px `resources/icon.png`) and the QR card's 2px edge, where the same asset is scaled behind the card and clipped to a hairline (1px reads as almost no color, 3px starts looking like a picture frame) so the artwork's own red/blue reads as the card's edge. This is a promotion surface for the mobile app, so showing the app's own icon is identification, not decoration.
- **Constraints**: the color comes only from the official asset — no component-authored gradient or brand hex; the edge layer carries no `filter` / `backdrop-filter` / `mask-image`; its motion is governed by §14.4. The card itself stays flat: **no shadow and no pointer 3D tilt** (both were tried and removed 2026-07-25 — they are not part of this exception).
- **Scope boundary**: this dialog only. Other working-UI surfaces keep the neutral treatment; a new brand surface needs a new entry here. Contract test: `mobileDownloadDialog.test.tsx` → `keeps the brand edge inside its registered exception`.
- **Reference renders** (light theme, zh-CN, @2x — `assets/mobile-download-dialog/`): [local mode](assets/mobile-download-dialog/guest-local.webp) (QR only — no account, so no permission card) · [signed in, no linked mobile](assets/mobile-download-dialog/onboarding.webp) (228px QR + permission card) · [signed in, mobile linked](assets/mobile-download-dialog/linked.webp) (132px QR + device list). Re-shoot these when the dialog's layout changes.
