...
| 英文 | zh-CN | ja | ko | 禁用译法 |
| --- | --- | --- | --- | --- |
| **Agent** | `Agent`（保留英文） | エージェント | 에이전트 | zh-CN: 智能体 |
| **Automation** | 自动化 | オートメーション | 자동화 | — |
| **Balance** | 余额 | 残高 | 잔액 | — |
| **Chat** | 对话 | チャット | 채팅 | zh-CN: 聊天 |
| **Credits** | 点数 | クレジット | 크레딧 | zh-CN: 额度 |
| **Effort** | 推理强度 | 推論強度 | 추론 강도 | zh-CN: 思考强度；zh-CN: 思考深度；zh-CN: 推理深度；zh-CN: 努力程度；zh-CN: 推理力度 |
| **Issue** | `Issue`（保留英文） | `Issue`（保留英文） | `Issue`（保留英文） | zh-CN: 工单；zh-CN: 议题 |
| **Plugin** | 插件 | プラグイン | 플러그인 | zh-CN: Plugin；zh-CN: plugin |
| **Project** | 项目 | プロジェクト | 프로젝트 | zh-CN: Project |
| **Provider** | 供应商 | プロバイダー | 제공업체 | zh-CN: 提供方；zh-CN: 提供商 |
| **Proxy** | `Proxy`（保留英文） | プロキシ | 프록시 | zh-CN: 代理 |
| **Quota** | 配额 | クォータ | 쿼터 | — |
| **Session** | 对话 | セッション | 세션 | zh-CN: 会话 |
| **Skill** | 技能 | スキル | 스킬 | — |
| **SkillHub** | `SkillHub`（保留英文） | `SkillHub`（保留英文） | `SkillHub`（保留英文） | — |
| **Subagent** | `Subagent`（保留英文） | サブエージェント | 하위 에이전트 | — |
| **Thread** | 对话 | スレッド | 스레드 | zh-CN: 线程 |
| **Usage** | 用量 | 使用量 | 사용량 | zh-CN: 使用情况；zh-CN: 使用表现 |
...
- **Agent** — 产品一级概念，中文保留英文原词（现状 44:5 已是主流）。「代理」一词在中文 UI 里整体废弃——Agent 与 Proxy 都保留英文，不共用这个译名（2026-07 裁决）；该词的 forbidden 规则统一登记在 proxy 条目下，避免同一处违规被两个术语重复报告。引入术语表时全仓仅 9 处「代理」，已全部清理为 Agent / Subagent / Proxy。ja / ko 沿用既有片假名 / 한글 音译，不受本条约束。豁免 settings.remote 整段：那里的 agent 指 ssh-agent（英文原文即 "SSH agent" / "probe agent"），是与产品 Agent 同形的另一个概念。
  - 豁免范围：`desktop:settings.remote.`
- **Automation** — 模块名 / 能力名用「自动化」（现状 88 处主流）；指单条实例时用「自动任务」，这是刻意保留的场景差异——一刀切会写出「查看你的自动化」这类别扭中文。特别注意 Automation 在本产品里指两个不同概念：scheduler 的定时任务，以及 settings.computerUse 的操控电脑能力；后者用「自动操作电脑 / 浏览器」，改成「自动化电脑」反而不通。因此不设 forbidden。
...
- **Issue** — 保留英文（2026-07 裁决）。现状「工单」32 /「议题」4 是既有 bug 的来源：列表按钮写「新建议题」、点进去页面标题写「新建工单」。「问题」不入 forbidden——它在「修复问题和失败」这类泛指语境下是正常中文，不是 Issue 的误译。checkCase 关闭：issue 同时是常用英语单词，prompt 模板里的 "fix the issue" 是正常用法，做大小写约束会制造 9 处假阳性。
...
- **Provider** — 中文用「供应商」（现状 36:18 领先简称）。全称「模型供应商」在需要消歧的位置仍可用。「提供商」「提供方」是同义漂移，禁用——设置页标题写「模型供应商」、它自己的表单写「提供商」是既有 bug。豁免 login.ssoVerificationSubtitle：那里的「身份提供方」是 SSO / SAML 领域 Identity Provider 的标准中文术语，与模型供应商是两个概念。
  - 豁免范围：`desktop:login.ssoVerificationSubtitle`
...
- **Quota** — 周期内的使用上限用「配额」（现状已有 3 处这么写）。与 credits 条目一起裁决：「额度」原先同时表示 Balance / Credits / Quota，三个概念在计费与限额提示里互相污染。「额度」的 forbidden 统一登记在 credits 条目下，不在这里重复——一个词只归属一个术语，否则同一处违规会被两个术语各报一次。
...
- **Skill** — 中文用「技能」（现状 32:26 领先）。偶尔保留英文 Skill 是允许的，因此不设 forbidden——硬性禁用会逼出别扭的中文。SkillHub 作为产品名保留英文，单独登记在 skillhub 条目。
