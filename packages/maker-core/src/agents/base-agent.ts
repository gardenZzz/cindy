...
export interface AgentDeps {
  ...
  /**
   * Host MCP risk classification shared by Claude Code, Codex, Pi and Cursor.
   ...
   *  - codex/index.ts mcpServerElicitation handler checks this before dispatchInteraction;
   *  - claude-code/index.ts canUseTool checks `mcp__<server>__<tool>` tools here;
   *  - pi adapter uses the same host policy for MCP tools;
   *  - cursor/index.ts session/request_permission checks tools that map to a registered MCP server.
   */
