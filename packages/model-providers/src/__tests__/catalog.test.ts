...
function registryEntryForRoute(providerId: string, modelId: string) {
  ...
  const entry = expandedRegistryEntries(registry).find((candidate) =>
    candidate.routes.some(
      (route) => route.providerId === providerId && route.modelId === modelId,
    ),
  );
  if (!entry)
    throw new Error(`missing registry route ${providerId}/${modelId}`);
  return entry;
}
  ...
  for (const p of clone.providers) {
    ...
    if (p.id === "openai") {
      ...
      p.models["claude-code"] = [
        model("chatgpt/gpt-5.5", { name: "GPT-5.5", supportsFastMode: true }),
      ];
    }
    if (p.id === "xd") {
      p.models["claude-code"] = [
...
describe("bundled catalog validity (dynamic-first contract)", () => {
  ...
  it("keeps selectable registry efforts self-consistent with defaultEffort", () => {
    ...
    for (const entry of expandedRegistryEntries(registry!)) {
      for (const agent of new Set(
        entry.routes.flatMap((route) => route.agents),
      )) {
        ...
        if (entry.defaultEnabled === false && defaultEffort === undefined)
          continue;
    ...
    expect(Date.parse(registry!.updatedAt)).toBeGreaterThan(
  ...
  it("has exactly the built-in providers in stable order", () => {
    ...
    expect(BUNDLED_CATALOG.providers.map((p) => p.id)).toEqual([
      "anthropic",
      "openai",
      "xai",
      "xd",
    ...
    for (const id of DYNAMIC_PROVIDER_IDS) {
      ...
      for (const agent of p.agents) {
        if (agent === "pi" && (id === "anthropic" || id === "openai")) {
          expect(
            p.models.pi?.length,
            `${id} must ship Pi native models`,
          ).toBeGreaterThan(0);
        } else {
          expect(
            p.models[agent],
            `${id} models[${agent}] must exist (empty array)`,
          ).toEqual([]);
        }
      }
    }
  ...
  it("ships Pi-native subscription models plus explicit Astra support, independent of Registry", () => {
    ...
    expect(
      provider("openai").models.pi?.some(
  ...
  it("xai ships a static fallback list and Pi official metadata", () => {
    ...
    expect(
      xai.models["claude-code"]?.find((m) => m.id === "xai/grok-4.6"),
    ).toMatchObject({
      contextWindow: 500_000,
  ...
  it("declares native Responses custom-tool support on each built-in Codex Responses route", () => {
    expect(provider("openai").routing.codex?.supportsResponsesCustomTools).toBe(
      true,
    );
    expect(provider("xd").routing.codex?.supportsResponsesCustomTools).toBe(
      false,
    );
    expect(provider("xai").routing.codex?.supportsResponsesCustomTools).toBe(
      false,
    );
  });

  it("declares access separately from model names", () => {
    expect(provider("anthropic").access).toEqual({
      kind: "subscription",
      product: "Claude.ai",
    });
    expect(provider("openai").access).toEqual({
      kind: "subscription",
      product: "ChatGPT",
    });
    expect(provider("xai").access).toEqual({
      kind: "subscription",
      product: "SuperGrok",
    });
    ...
    for (const p of BUNDLED_CATALOG.providers) {
      ...
      for (const models of Object.values(p.models)) {
        expect(
          models?.every(
            (m) => !m.name.includes(product) && !m.name.includes("订阅"),
          ),
        ).toBe(true);
      }
    }
  ...
  it("accepts the four portable piApi values and rejects unknown PI protocols", () => {
    for (const piApi of [
      ...
      catalog.providers.find((provider) => provider.id === "xai")!.models[
        "claude-code"
      ]![0]!.piApi = piApi;
    ...
    const bad = JSON.parse(JSON.stringify(BUNDLED_CATALOG)) as Catalog;
    (
      bad.providers.find((provider) => provider.id === "xai")!.models[
        "claude-code"
  ...
  it("ships Codex support metadata for the current XD gateway model set", () => {
    ...
    for (const [id, name] of Object.entries(expected)) {
      ...
      const route = entry.routes.find(
        (candidate) =>
          candidate.providerId === "xd" && candidate.modelId === id,
      );
    ...
    expect(
      registryEntryForRoute("xd", "bytedance-seed/seed-2.1-pro"),
    ).toMatchObject({
    ...
    expect(
      registryEntryForRoute("xd", "qwen/qwen3.8-max-preview"),
    ).toMatchObject({
    ...
    for (const id of [
      ...
      expect(registryEntryForRoute("xd", id), id).not.toHaveProperty(
        "description",
      );
    }
  });
  ...
  it("rejects legacy or ad-hoc top-level metadata blocks", () => {
    const bad = JSON.parse(JSON.stringify(BUNDLED_CATALOG)) as Record<
      string,
      unknown
    >;
    bad.cindyModelMeta = { version: 1, models: {} };
    expect(() => parseCatalog(bad)).toThrow(
      /catalog\.cindyModelMeta is not allowed/,
    );
  });
  ...
  it("models are grouped per-agent (no flat array, no rogue agent keys)", () => {
    for (const p of BUNDLED_CATALOG.providers) {
      expect(
        ...
        `${p.id} models must be a per-agent map`,
      ).toBe(false);
...
describe("routing modelPrefixes 服务范围契约 (issue #886)", () => {
  ...
  it("桥接型动态供应商(openai cc)的 modelPrefixes 保持声明(清单为空时范围门依然生效)", () => {
    ...
    expect(provider("openai").routing["claude-code"]?.modelPrefixes).toEqual([
      "chatgpt/",
    ]);
    expect(provider("xai").routing["claude-code"]?.modelPrefixes).toEqual([
      "xai/",
    ]);
    ...
    for (const p of BUNDLED_CATALOG.providers) {
      for (const agent of p.agents) {
        for (const prefix of p.routing[agent]?.modelPrefixes ?? []) {
          expect(
            prefix,
            `${p.id} routing[${agent}] prefix '${prefix}'`,
          ).toMatch(/^[a-zA-Z0-9_-]+\/$/);
        }
      }
    }
...
describe("routing wireProtocol per-agent 契约", () => {
  ...
  it("parseCatalog 允许 codex 使用 anthropic-messages(由本地 Responses→Anthropic bridge 接管)", () => {
    ...
    expect(
      parseCatalog(bad).providers.find((p) => p.id === "xai")?.routing.codex
        ?.wireProtocol,
...
describe("runtime-injected registry semantics(生产形态:动态清单注入后)", () => {
  ...
  it("claude 模型 cc 双来源(anthropic+xd);gpt cc 单来源(xd);gpt codex 双来源(openai+xd)", () => {
    ...
    expect(
      sourcesForModel(views, "gpt-5.5", "claude-code").map((p) => p.id),
    ).toEqual(["xd"]);
    expect(
      sourcesForModel(views, "gpt-5.5", "codex")
        .map((p) => p.id)
        .sort(),
    ).toEqual(["openai", "xd"]);
  });
  ...
  it("xai/grok 模型只经 xAI 提供(bundled 静态清单)", () => {
    expect(
      sourcesForModel(views, "xai/grok-4.3", "codex").map((p) => p.id),
    ).toEqual(["xai"]);
    expect(
      sourcesForModel(views, "xai/grok-4.3", "claude-code").map((p) => p.id),
    ).toEqual(["xai"]);
  });
  ...
  it("未注入时(bundled 原样)动态供应商零来源——不用静态数据冒充", () => {
    ...
    expect(
      sourcesForModel(bare, "xai/grok-4.3", "codex").map((p) => p.id),
    ).toEqual(["xai"]);
  });
});
...
describe("fast-mode per-provider resolution (model-level SSoT)", () => {
  it("modelSupportsFastMode reads the specific provider entry (false on missing provider/model)", () => {
    ...
    expect(
      modelSupportsFastMode(anthropic, "claude-opus-4-8", "claude-code"),
    ).toBe(true);
    expect(modelSupportsFastMode(xd, "claude-opus-4-8", "claude-code")).toBe(
      false,
    );
    expect(modelSupportsFastMode(anthropic, "gpt-5.5", "claude-code")).toBe(
      false,
    );
    expect(
      modelSupportsFastMode(undefined, "claude-opus-4-8", "claude-code"),
    ).toBe(false);
    expect(
      modelSupportsFastMode(anthropic, "claude-sonnet-4-6", "claude-code"),
    ).toBe(false);
  });
      ...
      providers: [
        {
          ...
          models: {
            "claude-code": [
              model("m1", { name: "M1", supportsFastMode: true }),
            ],
          },
        },
        {
          ...
          models: {
            "claude-code": [
              model("m1", { name: "M1", supportsFastMode: false }),
            ],
          },
        },
      ],
...
describe("vendor grouping metadata (xai fallback metadata)", () => {
  ...
  it("静态清单的 contextWindow 标记为已核实(可用于收敛上报值)", () => {
    ...
    for (const agent of xai.agents) {
      for (const m of xai.models[agent] ?? []) {
        ...
        expect(m.contextWindowVerified, `${m.id} contextWindowVerified`).toBe(
          true,
        );
      }
    }
  ...
  it("parseCatalog 给远端下发的静态条目补标记,但不覆盖显式表态", () => {
    ...
    expect(
      models.find((m) => m.id === "remote/known")?.contextWindowVerified,
    ).toBe(true);
    expect(
      models.find((m) => m.id === "remote/opted-out")?.contextWindowVerified,
    ).toBe(false);
  });
});
...
describe("provider OAuth and upstream URL validation", () => {
  ...
  it("rejects an OAuth descriptor on a non-OAuth auth method", () => {
    ...
    expect(() => parseCatalog(catalog)).toThrow(
      /auth\.oauth not allowed for none method/,
    );
  });

  it("rejects an upstream URL with embedded credentials", () => {
    const catalog = oauthCatalog();
    catalog.providers[0]!.routing.codex!.upstream =
  ...
  it.each([
    ...
    [
      "cross-origin base URL",
      { baseUrl: "https://other.example/v1", wireProtocol: "openai-responses" },
    ],
    [
    ...
    [
      "invalid protocol",
      { baseUrl: "https://api.example/v1", wireProtocol: "invalid" },
    ],
    [
      "unsafe request path",
      {
...
describe("buildRegistry 的清单发现失败投影", () => {
  const failure = {
    ...
    detail:
      'HTTP 403: {"error":{"type":"unsupported_country_region_territory"}}',
  };
  ...
  it("剥掉 detail 再下发 —— 原始上游响应体不出 Main", () => {
    ...
    const anthropic = views.find((p) => p.id === "anthropic");
    expect(anthropic?.modelDiscoveryFailure).toEqual({
      kind: "regionBlocked",
      at: failure.at,
    });
  ...
  it("没有失败态的供应商不长出该字段;稀疏 map 缺键也不报错", () => {
    ...
    expect(
      views.find((p) => p.id === "anthropic")?.modelDiscoveryFailure,
    ).toBeUndefined();
    const bare = buildRegistry(BUNDLED_CATALOG, { anthropic: true });
    expect(
      bare.find((p) => p.id === "anthropic")?.modelDiscoveryFailure,
    ).toBeUndefined();
  });
});
...
describe("媒体清单跨供应商契约(2026-07 图像多来源)", () => {
  ...
  it("embeddingModels / embeddingDefaults 与 image/video 同一套入口校验", () => {
    ...
    const withXd = (mutate: (xd: Record<string, unknown>) => void): Catalog => {
      const bad = JSON.parse(JSON.stringify(BUNDLED_CATALOG)) as Catalog;
      mutate(
        bad.providers.find((p) => p.id === "xd")! as unknown as Record<
          string,
          unknown
        >,
      );
