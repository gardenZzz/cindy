...
export const createWorkerSpecSchema = z.object({
  ...
  fast: z
    .boolean()
    .optional()
    .describe('可选, 是否给 worker 开启 Fast 模式。对 codex / cursor / pi worker 生效（且需所选模型支持 Fast）；claude-code 忽略。不传则继承默认。'),
  label: z
    .string()
    .trim()
    .min(1)
...
const DESCRIPTION = [
  ...
  '- agent: worker agent 类型 (codex / claude-code / cursor / pi)',
  ...
  '- fast: 可选 boolean, 是否给 worker 开启 Fast 模式 (更快输出)。用户明确说「fast / 快速 / 开/关 fast」时显式传。对 codex / cursor / pi worker 生效 (cursor / pi 需所选模型声明 supportsFastMode; 否则自动降级为关); claude-code worker 忽略此参数 (其 fast mode 在 agent 层为 no-op)。不传则继承默认 (New Maker 面板默认或 lead session 的 fastMode)。',
