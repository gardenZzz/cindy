...
type MakerVendor = 'cc' | 'codex' | 'pi' | 'orca' | 'cursor';
type DefaultTupleVendor = Extract<MakerVendor, 'cc' | 'codex' | 'pi'>;

export interface NewMakerDefaultTuple {
  vendor: DefaultTupleVendor;
  providerId: string;
  model: string;
  effort: Effort | null;
}
...
function vendorForAgent(agent: AgentKind): NewMakerDefaultTuple['vendor'] | null {
  if (agent === 'claude-code') return 'cc';
...
export function resolveNewMakerDefaultTuples(args: {
  ...
  // Method-shaped so extra installed agents (cursor) and orca do not trip Set invariance.
  availableAgents: { has(vendor: DefaultTupleVendor): boolean };
