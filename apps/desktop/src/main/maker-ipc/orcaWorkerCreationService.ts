...
function selectWorkerModel(params: {
  ...
  return input.model
    ...
    // cursor 走 CURSOR_DEFAULT_WORKER_MODEL;其它回落 claude-sonnet-4-6。
    ?? (input.agent === lead.agentKind ? lead.model
        : input.agent === 'codex' ? 'gpt-5.5'
        : input.agent === 'cursor' ? CURSOR_DEFAULT_WORKER_MODEL
...
function resolveWorkerConfig(params: {
  ...
  return {
    ...
    // cursor 无供应商路由维度(见 isProviderRoutedAgent):调用方已把 providerId 置 null。
    providerId,
...
/**
 * 会消费显式 `fast` 输入的 agent:Codex / Cursor / Pi(三者 agent 层都支持 Fast 模式)。
...
function agentConsumesExplicitFast(agent: AgentKind): boolean {
  return agent === 'codex' || agent === 'cursor' || agent === 'pi';
}
...
export function buildNoProviderMessage(
  ...
  const others = (['claude-code', 'codex', 'cursor', 'pi'] as AgentKind[]).filter(
...
export function createOrcaWorkerCreationService(deps: OrcaWorkerCreationDeps): OrcaWorkerCreationService {
  ...
  async function createWorkerInTeam(params: OrcaWorkerCreateInTeamParams): Promise<OrcaWorkerCreationResult> {
    ...
    // cursor 恒不带来源(见 isProviderRoutedAgent),不参与 defaults / Lead 的来源继承。
    ...
    const leadRouteModelResolution = providerRouted && explicitSourceId === null && leadProviderSnapshot
      ? resolveWorkerModelId({
          agent: params.agent,
          model: selectedModel,
    ...
    const inheritedProviderId = !providerRouted
      ? null
      : explicitSourceId
    ...
    const modelResolution = !providerRouted
      ? { ok: true as const, model: selectedModel }
      : compatibleLeadModelResolution
        ?? explicitModelResolution
        ?? resolveWorkerModelId({
          agent: params.agent,
          model: selectedModel,
    ...
    const defaultProviderId = providerRouted
      ? providerRouting.resolveDefaultProviderIdForModel(
          params.agent,
