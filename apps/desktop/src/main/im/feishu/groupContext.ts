 ...
 *   4. 统一防注入: 群主流与话题同一条拼装路径 —— fence 中和 + 未受信任
 *      第三方数据警告, 与群轮次强确认策略(adapter 侧)配套。
 *
...
export async function buildFeishuGroupContext(args: {
  ...
  const { lane, triggerMessageId, question, deps } = args;
  ...
  const lines = picked.map(renderHistoryLine);
      ...
      if (isTextLikeFile(att)) {
        try {
          ...
          if (stat.size <= FILE_INLINE_MAX_BYTES && fileInlineChars < FILE_INLINE_TOTAL_MAX_CHARS) {
            ...
            const body = neutralizeFenceTags(raw.slice(0, budget));
