...
export function CompanionModelPicker({ visible, deviceId, route, onClose, onClosed, onSelect }: {
  ...
  const select = async (config: MobileModelConfiguration) => {
    if (config.agent !== 'claude-code' && config.agent !== 'codex' && config.agent !== 'pi') return false;
    return onSelect({ harness: config.agent === 'claude-code' ? 'claude' : config.agent, model: config.modelId, providerId: config.providerId, effort: config.effort, fastMode: config.fast });
  };
